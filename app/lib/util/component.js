...
const getCp = (map, node, parent, index) => {
  if (!node) {
    return;
  }
...
const outerSimilarity = (cp, target) => {
  return 0;
};

const selfSimilarity = (cp, target) => {
  // if()
  const weightTag = 1;
  const weightType = 1;
  return (
    (weightTag * (cp.info.tag == target.info.tag ? 1 : 0) +
      weightType * (cp.info.type == target.info.type ? 1 : 0)) /
    (weightTag + weightType)
  );
};

const innerSimilarity = (cp, target, usingCssSimilarity) => {
...
const getSimilarity = (cp, target, usingCssSimilarity) => {
  // const weightInner = 1
  // const weightOuter = 0
  // const weightSelf = 0
  // return outerSimilarity(cp, target) * weightOuter + selfSimilarity(cp, target) * weightSelf + innerSimilarity(cp, target) * weightInner
  let res =
    innerSimilarity(cp, target, usingCssSimilarity) *
    selfSimilarity(cp, target);
  return res;
};

const computeSimilarity = (cp, sourceCps, usingCssSimilarity) => {
  let max = 0;
  let tag = '';
...
const getLeafComponent = (node, withHTML = true) => {
  let map = new Map();
  let id = 0;
